            ...
            {selectedLibraryItem ? (
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden animate-fade-in-up">
                    <div className="p-4 md:p-6 border-b border-gray-200 flex justify-between items-start">
                        <div className="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4 items-start flex-1 min-w-0">
                            <img
                                src={selectedLibraryItem.thumbnail || 'https://placehold.co/128x80/e2e8f0/64748b?text=No+Image'}
                                alt="썸네일"
                                className="w-full md:w-32 h-auto md:h-20 object-cover rounded-lg shadow-md flex-shrink-0"
                                onError={(e) => e.target.src = 'https://placehold.co/128x80/e2e8f0/64748b?text=No+Image'}
                            />
                            <div className="flex-1 min-w-0">
                                <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-2 break-words text-left">{selectedLibraryItem.title}</h3>
                                <p className="text-gray-600 text-xs md:text-sm mb-2 truncate text-left">{selectedLibraryItem.uploader}</p>
                        ...
                        <button
                            ...
                            className="ml-2 md:ml-4 p-1 md:p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
                        >
                    ...
                    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
                        {/* 요약 내용 */}
                        <div>
                            <h4 className="text-sm md:text-base font-semibold text-gray-800 mb-2">요약 내용</h4>
                            <div className="bg-gray-50 rounded-lg p-3 md:p-4 text-sm md:text-base text-gray-700 leading-relaxed">
                        ...
                        {/* 사용자 메모 */}
                        <div>
                            <h4 className="text-sm md:text-base font-semibold text-gray-800 mb-2">내 메모</h4>
                            <textarea
                                value={userNotes}
                                onChange={(e) => setUserNotes(e.target.value)}
                                placeholder="이 영상에 대한 개인적인 메모를 작성해보세요..."
                                className="w-full border border-gray-300 rounded-lg p-3 md:p-4 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-sm md:text-base"
                                rows={4}
                            />
                            <div className="flex flex-col sm:flex-row gap-2 md:gap-3 mt-3">
                                <button
                                    onClick={() => handleSaveUserNotes(selectedLibraryItem.id, userNotes)}
                                    className="bg-blue-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-lg font-medium hover:bg-blue-600 transition-colors text-sm md:text-base"
                                >
                                    메모 저장
                                </button>
                                <button
                                    onClick={() => handleDeleteLibraryItem(selectedLibraryItem.id)}
                                    className="bg-red-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-lg font-medium hover:bg-red-600 transition-colors text-sm md:text-base"
                                >
                                    삭제
                                </button>
                                <button
                                    onClick={() => handleSetReminder(selectedLibraryItem)}
                                    className="bg-yellow-500 text-white py-2 md:py-3 px-4 md:px-6 rounded-lg font-medium hover:bg-yellow-600 transition-colors text-sm md:text-base"
                                >
                                    리마인더 설정
                                </button>
                            </div>
                ...
                <div className="space-y-4 md:space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 md:p-6 flex flex-col sm:flex-row gap-3 md:gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 md:pl-10 pr-3 md:pr-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-700 text-sm md:text-base"
                            />
                        </div>
                        <div className="flex-1 relative">
                    ...
                    {isSearching ? (
                        <div
                            className="text-center text-gray-500 p-6 md:p-8 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col items-center justify-center min-h-[200px] md:min-h-[300px]">
                            <Loader2 className="h-8 w-8 md:h-10 md:w-10 animate-spin text-red-500 mb-3 md:mb-4"/>
                            <p className="text-base md:text-lg font-medium">검색 중...</p>
                            <p className="text-xs md:text-sm">잠시만 기다려주세요.</p>
                        </div>
                    ) : libraryItems.length === 0 ? (
                        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 md:p-8">
                            <div className="text-center py-8 md:py-12">
                                <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-yellow-100 to-yellow-200 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
                                    <Library className="h-8 w-8 md:h-10 md:w-10 text-yellow-600" />
                                </div>
                                <h3 className="text-xl md:text-2xl font-bold text-gray-800 mb-3 md:mb-4">첫 번째 요약본을 저장해보세요!</h3>
                                ...
                                <div className="flex flex-col gap-3 md:gap-4 justify-center items-center">
                                    <button
                                        ...
                                        className="bg-yellow-500 text-white py-2 md:py-3 px-6 md:px-8 rounded-lg font-bold hover:bg-yellow-600 transition-colors transform hover:scale-105 shadow-md flex items-center space-x-2 text-sm md:text-base"
                                    >
                            ...
                            {libraryItems.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="p-3 md:p-4">
                                        <h3 className="text-sm md:text-lg font-semibold text-gray-800 truncate mb-1">
                                            {item.title}
                                        </h3>
                    ...
                    <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-gray-200">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 md:mb-6 space-y-3 sm:space-y-0">
                            <div className="flex items-center space-x-2 md:space-x-3">
                                <svg className="h-6 w-6 md:h-8 md:w-8 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                                </svg>
                                <div className="text-left">
                                    <h3 className="text-lg md:text-2xl font-bold text-gray-800">라이브러리 통계</h3>
                                    <p className="text-gray-600 text-xs md:text-sm">태그별 요약 영상 분포를 한눈에 확인하세요</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setShowTagStats(!showTagStats)}
                                className={`px-4 md:px-6 py-2 md:py-3 rounded-lg font-medium transition-all duration-300 hover:scale-105 text-sm md:text-base ${
                                    showTagStats 
                        ...
                        {showTagStats && (
                            <div className="space-y-4 md:space-y-6 animate-fade-in">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 text-blue-800 rounded-xl p-4 md:p-6 shadow-lg border border-blue-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-blue-700 text-xs md:text-sm font-medium mb-1">총 영상 수</p>
                                                <p className="text-2xl md:text-3xl font-bold">{libraryItems.length}</p>
                                                <p className="text-xs text-blue-600 mt-1">저장된 요약 영상</p>
                                            </div>
                                            <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-200 rounded-full flex items-center justify-center hover:scale-110 transition-transform duration-200">
                                    ...
                                    <div className="bg-gradient-to-br from-green-50 to-green-100 text-green-800 rounded-xl p-4 md:p-6 shadow-lg border border-green-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-green-700 text-xs md:text-sm font-medium mb-1">사용된 태그</p>
                                                <p className="text-2xl md:text-3xl font-bold">{tagChartData.length}</p>
                                                <p className="text-xs text-green-600 mt-1">다양한 카테고리</p>
                                            </div>
                                            <div className="w-10 h-10 md:w-12 md:h-12 bg-green-200 rounded-full flex items-center justify-center hover:scale-110 transition-transform duration-200">
                                    ...
                                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 text-purple-800 rounded-xl p-4 md:p-6 shadow-lg border border-purple-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <p className="text-purple-700 text-xs md:text-sm font-medium mb-1">가장 인기 태그</p>
                                            ...
                                            <div className="w-10 h-10 md:w-12 md:h-12 bg-purple-200 rounded-full flex items-center justify-center hover:scale-110 transition-transform duration-200">
                                ...
                                <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 md:p-6">
                                    <div className="flex items-center space-x-2 md:space-x-3 mb-4 md:mb-6">
                                        ...
                                        <h4 className="text-base md:text-lg font-semibold text-gray-800">태그별 분포</h4>
                                    </div>
                                    ...
                                    {tagChartData.length > 0 ? (
                                        <div className="space-y-4 md:space-y-6">
                                            ...
                                            <div className="relative">
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ zIndex: 1 }}>
                                                    <div className="text-center bg-white bg-opacity-70 rounded-full p-3 md:p-4 shadow-lg">
                                                        <div className="text-lg md:text-2xl font-bold text-gray-700">
                                                            {tagChartData.length}
                                                        </div>
                                                        <div className="text-xs md:text-sm text-gray-500">총 태그</div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-6 md:mt-8">
                                                <h5 className="text-sm md:text-md font-semibold text-gray-700 mb-3 md:mb-4 flex items-center">
                                                    <svg className="h-3 w-3 md:h-4 md:w-4 mr-2 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between mb-2 md:mb-3">
                                                                    <div className="flex items-center space-x-2">
                                                                        <div 
                                                                            className="w-3 h-3 md:w-4 md:h-4 rounded-full transition-transform duration-200"
                                                                            style={{ backgroundColor: improvedColors[index % improvedColors.length] }}
                                                                        ></div>
                                                                        <span className="font-medium text-sm md:text-base text-gray-800 truncate">{tag.name}</span>
                                                                    </div>
                                                                    <span className="text-sm md:text-base font-bold text-gray-700">{tag.value}개</span>
                                                                </div>
                                        ...
                                        <div className="text-center py-8 md:py-12">
                                            <p className="text-gray-500 text-sm md:text-base">태그 데이터가 없습니다.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
